...
def is_valid_file(file_path):
    ...
    try:
        ...
        if file_ext not in valid_extensions:
            logging.error(f"Invalid file extension: {file_ext}. Supported extensions: {', '.join(valid_extensions)}")
            return False
...
class OCRThread(QThread):
    ...
    def run(self):
        try:
            ...
            error_msg = f"Error in OCR process: {str(e)}"
            logging.error(error_msg)
            self.debug.emit(error_msg)
...
class CustomFileDialog(QDialog):
    def __init__(self, parent=None):
        ...
        self.filter_combo.addItems(["All Files", "PDF Files", "Image Files"])
        ...
        try:
            ...
            for file in files:
                ...
                if os.path.isfile(full_path):
                    ...
                    if self.filter_combo.currentText() == "All Files":
                        self.file_list.addItem(file)
                    elif self.filter_combo.currentText() == "PDF Files" and ext == '.pdf':
                        self.file_list.addItem(file)
                    elif self.filter_combo.currentText() == "Image Files" and ext in ['.png', '.jpg', '.jpeg']:
                        self.file_list.addItem(file)
        except Exception as e:
...
class MainWindow(QMainWindow):
    ...
    def browse_file(self):
        ...
        if dialog.exec():
            file_name = dialog.selected_file
        ...
        try:
            ...
            if file_ext == '.pdf':
                ...
                if not is_valid_file(file_name):
                    raise ValueError("File validation failed. Please check the debug log for details.")
            ...
            QMessageBox.warning(
                ...
                "2. The file is one of the supported formats (PNG, JPG, JPEG, PDF)\n"
